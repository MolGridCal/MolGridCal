...
public class GridFUCSFDOCKTask extends AbstractTask<String> {
	String CODING_1 = "GBK";
	...
	public GridFUCSFDOCKTask(String ip, int port, String user, String passwd, String downdir, String uploaddir,
	...
	public void gridFDockTask(String ipAdress, int port, String user, String passwd, String downdir, String uploaddir,
		...
		System.out.println("###################################################################" + "\n"
				+ "# This work is performed by UCSF Dock for virtual screening.      #" + "\n"
		...
		try {
			fip = new FileInputStream(userdir + "/UCSFDock.in");
			isr = new InputStreamReader(fip);
			data = new BufferedReader(isr);
		} catch (FileNotFoundException e3) {
			System.out.println("UCSFDock.in cannot be found!");
			e3.printStackTrace();
		}

		String rl;
		// Story data for modify.
		try {
			for (;;) {
				rl = data.readLine();
		...
		try {
			writer = new OutputStreamWriter(new FileOutputStream(userdir + "/UCSFDock.in"));
		...
		// Replace the keywords for different ligands.
		for (int i = 0; i < tmpArr.size(); i++) {
			if (tmpArr.get(i).contains("ligand_atom_file")) {
				pWriter.printf("ligand_atom_file                                             " + fltmp + '\n');
				continue;
			} else if (tmpArr.get(i).contains("vdw_defn_file")) {
				pWriter.printf("vdw_defn_file                                                " + commandDir
						+ "/parameters/vdw_AMBER_parm99.defn" + '\n');
				continue;
			} else if (tmpArr.get(i).contains("flex_defn_file")) {
				pWriter.printf("flex_defn_file                                              " + commandDir
						+ "/parameters/flex.defn" + '\n');
				continue;
			} else if (tmpArr.get(i).contains("flex_drive_file")) {
				pWriter.printf("flex_drive_file                                                " + commandDir
						+ "/parameters/flex_drive.tbl" + '\n');
				continue;
			} else if (tmpArr.get(i).contains("ligand_outfile_prefix")) {
				String[] tmpStr2 = fltmp.split("\\.");
				pWriter.printf(
						"ligand_outfile_prefix                                              " + tmpStr2[0] + '\n');
				continue;
			}
		...
		if (commandDir.endsWith("/")) {
			command = commandDir + "bin/" + command;
		} else {
			command = commandDir + "/bin/" + command;
		}
		...
		while (token1) {
			...
			if (GridFDockRunner.cn.check(command) == 0) {
				...
				try {
					...
					while (tk1 && j <= 10) {
						// Upload result.
						GridFDockRunner.dt.upload(userdir + "/" + tmpStr[0] + "_scored.mol2",
								uploaddir + "/" + tmpStr[0] + "_scored.mol2");
						...
						remoteFile = GridFDockRunner.dt.getFileSize(ipAdress, user, passwd, uploaddir,
								tmpStr[0] + "_scored.mol2", port);
						localFile = getLocalFileSize(userdir, tmpStr[0] + "_scored.mol2");
						System.gc();
						if (remoteFile > localFile) {
							...
							GridFDockRunner.dt.deleteFtpServerFile(uploaddir + "/" + tmpStr[0] + "_scored.mol2");
				...
				try {
					deleteFile(userdir, tmpStr[0] + "_scored.mol2");
				} catch (Exception e) {
					e.printStackTrace();
				...
				try {
					...
					System.out.println("Sleep interrupted exception while waiting for uploading ligand.");
					e.printStackTrace();
				}
			}
		}
